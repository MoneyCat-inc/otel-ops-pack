# Robust Production SSOT Monitoring
# Enhanced production monitoring with better error handling and reporting
...
"=== Robust Production Monitoring Started - $timestamp ===" | Out-File -FilePath $LogPath -Append -Encoding UTF8

function Get-RobustHealthScore {
    $timestamp = Get-Date -Format 'yyyy-MM-ddTHH:mm:ssZ'
    
    Write-Host "🔄 Robust Health Check - $timestamp" -ForegroundColor Cyan
    
    try {
        # Run health check and capture all output
        $healthProcess = Start-Process -FilePath "pwsh" -ArgumentList @("-ExecutionPolicy", "Bypass", "-File", "scripts/monitor-ssot-health.ps1") -RedirectStandardOutput -RedirectStandardError -PassThru -Wait -WindowStyle Hidden -NoNewWindow
        
        $stdout = Get-Content $healthProcess.StandardOutput -Raw -ErrorAction SilentlyContinue
        $stderr = Get-Content $healthProcess.StandardError -Raw -ErrorAction SilentlyContinue
        $exitCode = $healthProcess.ExitCode
        ...
        if ($exitCode -eq 0) {
            ...
            if ($stdout -match "Overall Health: (\d+)%") {
                $healthScore = [int]$matches[1]
            } elseif ($stdout -match "Health Score: (\d+)%") {
                $healthScore = [int]$matches[1]
            } elseif ($stdout -match "(\d+)% \(excellent\)") {
                $healthScore = [int]$matches[1]
            }
            
            # Extract freshness status
            $freshness = "unknown"
            if ($stdout -match "Status: (\w+)") {
                $freshness = $matches[1]
            } elseif ($stdout -match "Freshness.*?(\w+)") {
                $freshness = $matches[1]
            }
...
function Update-SSOTBlockRobust {
    $timestamp = Get-Date -Format 'yyyy-MM-ddTHH:mm:ssZ'
...
function Generate-ProductionMetrics {
    ...
    Write-Host "📊 Production Metrics Generated:" -ForegroundColor Green
...
if ($Continuous) {
    Write-Host "🔄 Starting continuous robust production monitoring..." -ForegroundColor Yellow
    ...
    while ($true) {
        ...
        # Health check
        $healthResult = Get-RobustHealthScore
        $healthHistory += $healthResult
        
        # Update SSOT block
        $ssotUpdateSuccess = Update-SSOTBlockRobust
    ...
    Write-Host "🔄 Running single robust production health check..." -ForegroundColor Yellow
    ...
    # Summary
    Write-Host "`n📊 Robust Production Health Summary:" -ForegroundColor Cyan
    Write-Host "   Health Score: $($healthResult.HealthScore)%" -ForegroundColor $(if ($healthResult.HealthScore -ge 95) { 'Green' } elseif ($healthResult.HealthScore -ge 80) { 'Yellow' } else { 'Red' })
...
Write-Host "`n📝 Robust production monitoring log: $LogPath" -ForegroundColor Cyan
if ($GenerateMetrics) {
...
Write-Host "✅ Examine: Robust production SSOT state captured and monitored" -ForegroundColor Green
Write-Host "✅ Clean: Robust production monitoring system operational" -ForegroundColor Green
Write-Host "✅ Report: Robust monitoring results documented with evidence" -ForegroundColor Green
Write-Host "✅ Role: Cursor Agent (Observability Copilot) - Robust production monitoring" -ForegroundColor Green
