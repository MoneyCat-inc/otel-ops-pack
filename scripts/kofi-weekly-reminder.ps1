...
<#
.SYNOPSIS
  Weekly Ko-fi maintenance reminder for Resonai [OTel].
.DESCRIPTION
  Tracks cadence in artifacts/kofi-maintenance-state.json.
...
$StatePath = Join-Path $RepoRoot 'artifacts/kofi-maintenance-state.json'
$Runbook = 'docs/BossCat/KOFI_WEEKLY_MAINTENANCE.md'
$KofiUrl = 'https://ko-fi.com/fubumaki'
$TaskName = 'Resonai-Kofi-Weekly-Maintenance'
...
function Show-Checklist {
    Write-Status "`n=== Ko-fi weekly maintenance DUE ===" 'Yellow'
    Write-Status "Page: $KofiUrl"
    Write-Status "Runbook: $Runbook`n"
    Write-Status @'
Checklist (summary):
  [ ] Public profile loads; about + GitHub/Patreon links current
  [ ] One-time tips enabled; monthly/membership tiers sane (if on)
  [ ] Payouts (Stripe/PayPal) healthy; thank new supporters
  [ ] README Ko-fi badge URL matches live page
  [ ] Mark complete: pwsh -File scripts/kofi-weekly-reminder.ps1 -MarkComplete
'@ 'White'
}
...
if ($RegisterScheduledTask) {
    $scriptPath = Join-Path $RepoRoot 'scripts/kofi-weekly-reminder.ps1'
    $action = New-ScheduledTaskAction -Execute 'pwsh.exe' -Argument "-NoProfile -File `"$scriptPath`" -Notify"
    $trigger = New-ScheduledTaskTrigger -Weekly -DaysOfWeek Monday -At '10:15'
    $settings = New-ScheduledTaskSettingsSet -AllowStartIfOnBatteries -DontStopIfGoingOnBatteries
    Register-ScheduledTask -TaskName $TaskName -Action $action -Trigger $trigger -Settings $settings -Force | Out-Null
    Write-Status "Registered weekly task '$TaskName' (Mondays 10:15, -Notify)." 'Green'
    exit 0
}
...
if ($MarkComplete) {
    ...
    Save-State $payload
    Write-Status "Ko-fi maintenance marked complete. Next due: $($nextDue.ToLocalTime().ToString('yyyy-MM-dd HH:mm')) local." 'Green'
    exit 0
}
...
if ($daysUntil -gt 0) {
    Write-Status "Ko-fi maintenance OK — due in $([math]::Ceiling($daysUntil)) day(s) ($($nextDueUtc.ToLocalTime().ToString('yyyy-MM-dd')))" 'Green'
    if ($state.notes) {
...
Show-Checklist
Show-Toast 'Ko-fi maintenance due' 'Run scripts/kofi-weekly-reminder.ps1 — see docs/BossCat/KOFI_WEEKLY_MAINTENANCE.md'
exit 2
