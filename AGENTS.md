...
Four seats (OEM D2, 2026-07-26), plus the Claude Code local seat registered 2026-09-24 (operator decision) — do not route human mint/login work to the wrong one. The Kiro provisional tag **resolved to permanent on 2026-08-14**: all three criteria frozen before the report was read (`docs/BossCat/KIRO_VERDICT_CRITERIA_20260813.md`) passed on independent scoring, recorded in `CHAR/ECRR/ECRR_REPORTS/ECRR_KIRO_PILOT_20260814.md`.
...
| **Claude Code (local seat)** (registered 2026-09-24) | Claude Code on the operator's host, in the operator's checkout; implementer peer of Cursor and Kiro | Code, CI, docs, ECRR in lane; PRs and `gh` with existing auth; log `Actor: Claude Code (local seat)` per commit; executes a merge only on the operator's explicit go for that change | Nest with Cursor or Kiro; mint/auth/elevate; read a credential; self-authorize a merge |
