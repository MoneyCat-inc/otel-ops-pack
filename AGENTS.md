...
Four seats (OEM D2, 2026-07-26; **Cursor{Implementer} retired 2026-09-24** when the operator uninstalled Cursor, and the Claude Code local seat registered the same day took seat 3) — do not route human mint/login work to the wrong one. The Kiro provisional tag **resolved to permanent on 2026-08-14**: all three criteria frozen before the report was read (`docs/BossCat/KIRO_VERDICT_CRITERIA_20260813.md`) passed on independent scoring, recorded in `CHAR/ECRR/ECRR_REPORTS/ECRR_KIRO_PILOT_20260814.md`.
...
| **Kiro{Implementer}** (permanent, 2026-08-14) | Kiro CLI peer seat | Spec-shaped feature work; log `Actor: Kiro{Implementer}` per commit | Nest with another implementer seat; mint/auth; open a second (AWS) evidence plane; self-merge |
| **Claude Code (local seat)** (registered 2026-09-24) | Claude Code on the operator's host, in the operator's checkout; implementer peer of Kiro | Code, CI, docs, ECRR in lane; PRs and `gh` with existing auth; log `Actor: Claude Code (local seat)` per commit; executes a merge only on the operator's explicit go for that change | Nest with Kiro; mint/auth/elevate; read a credential; self-authorize a merge |
...
**Standing rule (routing):** Any step that needs a keyboard — password page, mint button, Secrets UI, or **starting an interactive agent session** — is **machine-operator only**. That seat is `@fubumaki`, the only human in the system. An implementer seat may open a visible terminal window to the right cwd, but the operator owns the session. Chat never “owns” the mint or the launch. Saying “blocked on Fae/chat” for `LUMI_API_KEY` (or for `kiro-cli chat`) is a routing bug — the handoff is: machine operator acts → tells the implementer seat → it verifies and logs.

**Standing rule (implementer seats):** Kiro and the Claude Code local seat are **peers**, not a nested chain (Cursor{Implementer} retired 2026-09-24). No seat→seat→model telephone game for pilot implementation. Scoped credentials only; machine operator handles auth; actor logged per commit.
...
- Auto-Bots registry (generated): docs/BossCat/AGENTS.md
  - Generated by `BRAV/SCPT/agent/setup-bots.ts` (`pnpm tsx BRAV/SCPT/agent/setup-bots.ts`); the `pnpm agent:setup` alias was removed from package.json (audit P3, 2026-08-29). Historical Tetragram/IONA roster — live seats are in docs/BossCat/CHARTER.md
