# Schedule monitoring script without #Requires directive
...
if (-not $isAdmin) {
    Write-Host "[schedule] ✗ This script requires administrator privileges" -ForegroundColor Red
    Write-Host "[schedule] Please run PowerShell as Administrator and try again" -ForegroundColor Yellow
    Write-Host "[schedule] Or run: Start-Process PowerShell -Verb RunAs -ArgumentList '-File `"$PSCommandPath`"'" -ForegroundColor Cyan
    exit 1
}

Write-Host "[schedule] ✓ Running with administrator privileges" -ForegroundColor Green
