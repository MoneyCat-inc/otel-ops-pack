...
**Rationale**: The repository already had a well-structured agent system, so we extended it rather than replacing it.
This maintains compatibility with existing workflows while adding the required Local Workflow Custodian capabilities.
...
**Rationale**: Following the existing pattern of PowerShell script integration in package.json. This provides familiar
entry points for developers while maintaining the Windows/PowerShell focus of the project.
...
**Rationale**: Simple, reliable emergency brake mechanism. File presence/absence is atomic and doesn't require complex
state management. All scripts check for lock before proceeding with any operations.

**Implementation**: All three core scripts check for `.agent/LOCK` at startup and exit immediately if found, updating
status to "paused:lock".
...
**Rationale**: These are the most common security and accessibility violations that can be automatically detected and
reported. The agent focuses on detection and reporting rather than automatic fixing to avoid breaking working code.
...
**Rationale**: Background maintenance should be non-intrusive but reliable. The cyclical approach allows for regular
health checks while processing queued tasks without overwhelming the system.
...
**Rationale**: Machine-readable logs enable programmatic monitoring and integration, while human-readable logs provide
transparency for developers. This follows the ECRR (Examine → Clean → Report → Role) methodology.
...
**Rationale**: The existing codebase is Windows-focused with PowerShell integration. Maintaining this approach ensures
consistency and leverages existing Windows-specific features like service management.
...
**Rationale**: Local development environments need to be stable and predictable. Scripts should be safe to run multiple
...
The implementation leverages the existing agent infrastructure while adding Local Workflow Custodian capabilities.
