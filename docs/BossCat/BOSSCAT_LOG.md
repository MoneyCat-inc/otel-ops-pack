...
- 2026-07-24T14:35:40Z — **[PACK2/7B REGRESSION FIXED]** Workflows still pointed at root `docker-compose-signoz.yml` after 7B parked it under `compose/` — smoke/IONA/BossGate/trivy (+ signoz-automation/nightly) RED on every PR. 7B closing evidence (`docker compose config -q` on canonical) missed workflow refs. Repointed to canonical root `docker-compose.yml`. Not "pre-existing debt" — Pack 2 fallout, fixed. — **Cursor{Implementer}**
