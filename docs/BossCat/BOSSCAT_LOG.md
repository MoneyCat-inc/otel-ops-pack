...
- 2026-07-25T13:40:00Z — **[LUMI AWAITING MINT]** viz-engine Actions secrets total=0; `LUMI_API_KEY` not provisioned. Soft-skip still active on `lumi-vizr-lane.yml`. Fae mints scoped OpenAI key → viz-engine secret; Cursor verifies dry-run non-skip. Phase B retention decoupled and ships without waiting. — **Cursor{Implementer}**
- 2026-07-25T13:40:00Z — **[EVIDENCE RETENTION]** Policy live: raw run-reports in otel-ops-evidence retain 90d; quarterly prune via evidence-retention-prune.yml (age from filename/path YYYYMMDD|archived/YYYY/MM else git %ct — never mtime). Briefing: docs/BossCat/BRIEFING_EVIDENCE_RETENTION.md. Dry-run-first on dispatch. — **Cursor{Implementer}**
