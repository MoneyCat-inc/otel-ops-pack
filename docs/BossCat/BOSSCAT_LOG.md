...
- 2026-07-24T14:40:00Z — **[GATE-DEF CONTRACT]** Pre-Pack-3 re-audit of required-check-shims.yml: package-only Dependabot tips reported Gate/Site required contexts SUCCESS (e.g. run 30100444751 on a3fc3e5). #356 unfiltered-primary claim holds — shim re-introduction condition NOT met. Prior "MISSING" was U+2022 poll encoding; UNSTABLE was Pack 2 7B compose-path RED (#360). Zero shim jobs retained (latest-wins hazard). — **Cursor{Implementer}**
- 2026-07-24T14:35:40Z — **[PACK2/7B REGRESSION FIXED]** Workflows still pointed at root `docker-compose-signoz.yml` after 7B parked it under `compose/` — smoke/IONA/BossGate/trivy RED. 7B closing evidence (`docker compose config -q` on canonical) missed workflow refs. Repointed gates to `compose/docker-compose-signoz.yml` (SigNoz-only; canonical root compose includes unpullable local images). Not pre-existing debt — Pack 2 fallout, fixed in #360. — **Cursor{Implementer}**
